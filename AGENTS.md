...
- Flux/Kustomize: mode-specific Flux sync roots under `clusters/*/{external,sops}` must render only the staged Flux `Kustomization` CRs (`platform-infrastructure`, `platform-secrets`, `platform-applications`); do not inline `../infrastructure` or `../apps` there or CRD consumers can dry-run before Helm installs their CRDs.
...
- Flux staging: `platform-secrets` must not depend on `platform-infrastructure` readiness in this repo; several infrastructure HelmReleases consume secrets during install. Keep `platform-applications` depending on both `platform-infrastructure` and `platform-secrets`.
- Flux staging: gateway-backed `RemoteMCPServer` objects must stay in `platform-applications`, and the AgentGateway/KMCP CR consumers (`infrastructure/network/kgateway/resources`, `apps/ai-gateway/agentgateway/resources`, `apps/platform/kmcp/resources`) must stay out of the `platform-infrastructure` Flux stage when their CRDs come from Helm in that same stage; otherwise Flux dry-run fails before those CRDs exist on a cold bootstrap.
...
- Kagent controller bootstrap: keep a startup wait on `agentgateway-proxy.agentgateway-system.svc.cluster.local:8080` before the controller container starts; this repo's gateway-backed `RemoteMCPServer` discovery can fail permanently if the controller races the proxy listener on a cold cluster.
