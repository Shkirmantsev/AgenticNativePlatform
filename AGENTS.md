...
- GitHub Actions GHCR publish: for repo image-publish workflows, treat Buildx `cache-to=gha` as optional (`ignore-error=true`) and scope it per image; in this repo cache export can fail after a successful GHCR push and should not make the release red.
