...
- Kagent grafana-mcp: `grafana.secretRef` must reference a Secret in the `kagent` namespace containing `GRAFANA_SERVICE_ACCOUNT_TOKEN` or `GRAFANA_API_KEY`; do not point it at `observability/observability-grafana-admin`, and keep the tool disabled by default until such a token secret is provisioned.
