...
- Kagent controller probes: the controller health endpoint binds on `:8082` while the API server binds on `:8083`; expose a dedicated `health` container port and point startup/readiness probes at that port, not the service port.
