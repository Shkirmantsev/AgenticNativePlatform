...
- Flux staging: gateway-backed `RemoteMCPServer` objects must stay in `platform-applications`, while their runtime dependencies (`infrastructure/network/kgateway/resources`, `apps/ai-gateway/agentgateway/resources`, `apps/platform/kmcp/resources`) belong in `platform-infrastructure`; otherwise kagent can reconcile remotes before `agentgateway-proxy:8080` is listening on a fresh bootstrap.
