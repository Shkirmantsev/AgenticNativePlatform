...
```bash
docker build -t finnhub-mcp-server:0.7.0 .
```
...
kmcp build \
  ...
  --tag ghcr.io/<github-owner>/finnhub-mcp-server:0.7.0
```
...
```text
ghcr.io/shkirmantsev/finnhub-mcp-server:0.7.0
```

If you already built a local image such as `finnhub-mcp-server:0.7.0`, you can retag and push it manually instead of rebuilding:

```bash
docker tag finnhub-mcp-server:0.7.0 \
  ghcr.io/<github-owner>/finnhub-mcp-server:0.7.0

docker push ghcr.io/<github-owner>/finnhub-mcp-server:0.7.0
```
